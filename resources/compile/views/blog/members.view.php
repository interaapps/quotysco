    ...
    <app>
                    ...
                    <div class="members_member">
                        ...
                        <div style="margin-left: auto;">
                            @if(($member->rank != "OWNER"))#
                                <select user="{{$member->id}}" class="user_rank">
                                    <option value="ADMIN" @if(($member->rank=="ADMIN"))#selected@endif>ADMIN</option>
                                    <option value="WRITER" @if(($member->rank=="WRITER"))#selected@endif>WRITER</option>
                                </select>
                                <a class="members_member_delete btn red" user="{{$member->id}}" >Remove</a>
                            @endif
                        </div>
                    </div>
                @endforeach
        ...
        <script>
            $(".members_member_delete").click(function(){
                ...
                Cajax.post("/{{$blog["name"]}}/a/members/remove", {
            ...
            $(".user_rank").on("change", function(){
                console.log("deleting...");
                Cajax.post("/{{$blog["name"]}}/a/members/changerank", {
                    userid: $(this).attr("user"),
                    rank: $(this).val()
                }).then(function(res) {
                    const parsed = JSON.parse(res.responseText);
            ...
            $("#add_user").click(function(){
                Cajax.post("/{{$blog["name"]}}/a/members/add", {
